macroScript GCT category:"TurboSquid"
(
	-----------------------------------------------------------------------
	--CHECK-MATE 
	--Geometry Checker Tools
	--v0.2.1 - 2011/02/04
	...
	--v0.2.10 : 2011/05/25, JB, added check for and delete spacewrap modifiers commands
		-----------------------------------------------------------------------
	-- 2011/05/31 modified by Denys Almaral ;) (seach for DenysMod )
	-- v0.2.11 : 2011/05/31, Making it compatible with Max 2008 (would lose some features )
...
-----------------------------------------------------
	...
	(
		Version = "v0.2.10 : 2011/05/25 @5:00 pm",
	...
	(
		...
		(
			--DenysMod: add if condition
			if currMaxVersion>11 then
			(
				local theObject = GeometryCheckerTools_Rollout.allObjects[GeometryCheckerTools_Rollout.theSelection+1]
	...
	(
		...
		(
			local theResults = #()
			local returnValue =  try(IsolatedVertices.Check currentTime theObj &theResults)catch(#Failed)
		...
		(
			local theResults = #()
			returnValue =  try(TVerts.Check currentTime theObj &theResults)catch(#Failed)
			if returnValue == #Failed then 
				false
			else
				theResults		
		...
		(
			local theResults = #()
			local returnValue = try(OverlappingVertices.Check currentTime theObj &theResults)catch(#Failed)
			if returnValue == #Failed then 
				false
			else
				theResults		
		...
		(
			local theResults = #()
			local returnValue = try(OverlappingFaces.Check currentTime theObj &theResults)catch(#Failed)
			if returnValue == #Failed then 
				false
			else
				theResults		
...
-------------------------------------------
	...
	(
		...
		(
			...
			OverlappingFacestolerance = GeometryCheckerTools_Settings.OverlappingFacesTolerance
			OverlappingVerticestolerance = GeometryCheckerTools_Settings.OverlappingVertsTolerance
			...
			(
				...
				*/
				...
				local theVal = ((o.transform.scalepart) as string)			
				subLi = li.SubItems.add theVal
				subLi.name = "scale"
		...
		(
			...
			--DenysMod: removed dot OverlappingVertices.tolerance
			...
			append thePropertiesToCheck #("Total Overlapping Vertices (Tolerance:"+OverlappingVerticestolerance as string+")", (overlappingVerticesCountTOTAL as string + " of " + totalVertices as string + " ("+ (if totalVertices == 0 then 0 else (100.0*overlappingVerticesCountTOTAL/totalVertices)) as string+"%)" ), overlappingVerticesCountTOTAL>0 )
			...
			--DenysMod: removed dot: OverlappingFaces.tolerance		
			...
			append thePropertiesToCheck #("Total Overlapping Faces (Tolerance:"+OverlappingFacestolerance as string+")" , (overlappingFacesCountTOTAL as string + " of " + totalPolygons as string + " ("+ (if totalPolygons == 0 then 0 else (100.0*overlappingFacesCountTOTAL/totalPolygons)) as string+"%)" ), overlappingFacesCountTOTAL>0 )
		...
		(
			...
			local oldType = 0
			if CurrMaxVersion > 11 then oldType = viewport.getType index:oldView else OldType = viewport.getType
			--
			...
			(
				...
				(
					...
					--DenysMod:  added if condition, write blank value					
					if CurrMaxVersion > 11 then txt1 += HTMLCellBGColor + (viewport.getType index:i) as string + "</td>"
													else	txt1 += HTMLCellBGColor + " " + "</td>"
    ...
    --DenysMod
	-- Check for lower version: max 2008, also changed MessageBox string
	currMaxVersion = ((maxVersion())[1] / 1000)
	if currMaxVersion > 9 then
	(
	...
	else messagebox "This script requires 3ds Max 2008 or higher." title:"Geometry Checker Tools"
